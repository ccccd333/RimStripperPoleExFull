...
* ->売春の基本料金と美しさによる倍率設定
* ->体位による倍率設定
* ->ダンス時観客の所持シルバーがない分は無から生成するか
* ->売春時クライアントの所持シルバーがない分は無から生成するか
<バグ修正>
* 元Modからのバグで娯楽ゲージがダンサーと観客ともに増加していなかった。増加するように修正。
* UAPからのバグで、ポールダンス実行時に徴兵などすると強制的にポールの位置へテレポートしてしまうので修正。
